...
func (l *GetFriendCountLogic) GetFriendCount(in *pb.GetFriendCountReq) (*pb.GetFriendCountResp, error) {
	hLen, err := xredis.HLen(l.svcCtx.Redis(), l.ctx, rediskey.FriendList(in.CommonReq.Id))
	if err != nil {
		friendList, err := relationmodel.GetMyFriendList(l.ctx, l.svcCtx.Redis(), l.svcCtx.Mysql(), in.CommonReq.Id)
		if err != nil {
	...
	if hLen == 1 {
		// 是否是Not Found
		exist, err := l.svcCtx.Redis().HexistsCtx(l.ctx, rediskey.FriendList(in.CommonReq.Id), xredis.NotFound)
		if err != nil {
			l.Errorf("GetFriendCount failed, err: %v", err)
			return &pb.GetFriendCountResp{CommonResp: pb.NewRetryErrorResp()}, err
		}
