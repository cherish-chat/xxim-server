...
func (l *SetGroupMemberRoleLogic) SetGroupMemberRole(in *pb.SetGroupMemberRoleReq) (*pb.SetGroupMemberRoleResp, error) {
	...
	var group *groupmodel.Group
	{
		...
		if err != nil {
			...
			return &pb.SetGroupMemberRoleResp{CommonResp: resp.CommonResp}, err
		}
		value, ok := resp.GroupMap[in.GroupId]
		if !ok {
			return &pb.SetGroupMemberRoleResp{CommonResp: pb.NewAlertErrorResp(
	...
	if myRole == pb.GroupRole_MEMBER && in.MemberId != in.CommonReq.UserId {
		return &pb.SetGroupMemberRoleResp{
