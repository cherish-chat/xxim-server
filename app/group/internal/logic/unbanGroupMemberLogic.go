...
func (l *UnbanGroupMemberLogic) UnbanGroupMember(in *pb.UnbanGroupMemberReq) (*pb.UnbanGroupMemberResp, error) {
	...
	var group *groupmodel.Group
	{
		...
		if err != nil {
			...
			return &pb.UnbanGroupMemberResp{CommonResp: resp.CommonResp}, err
		}
		value, ok := resp.GroupMap[in.GroupId]
		if !ok {
			return &pb.UnbanGroupMemberResp{CommonResp: pb.NewAlertErrorResp(
	...
	if myRole == pb.GroupRole_MEMBER && in.MemberId != in.CommonReq.UserId {
		return &pb.UnbanGroupMemberResp{
