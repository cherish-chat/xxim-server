...
func (l *GetUserSettingsLogic) GetUserSettings(in *pb.GetUserSettingsReq) (*pb.GetUserSettingsResp, error) {
	...
	var err error
	if len(in.Keys) > 0 {
		err = l.svcCtx.Mysql().Model(&usermodel.UserSetting{}).Where("userId = ? and `key` in (?)", in.Requester.Id, in.Keys).Find(&models).Error
	} else {
		err = l.svcCtx.Mysql().Model(&usermodel.UserSetting{}).Where("userId = ?", in.Requester.Id).Find(&models).Error
	}
	if err != nil {
